...
model=dict(
    ...
    roi_head=dict(
        type='HybridTaskCascadeRoIHead_Lite_Partial',
        interleaved=True,
        mask_info_flow=True,
        num_stages=3,
        ...
        semantic_head=dict(
            ...
            conv_out_channels=64,
            num_classes=1,
            loss_seg=dict(type='PartialDiceLoss', loss_weight=1.0, use_sigmoid=True)),
    ),
    train_cfg=dict(
    rpn=dict(
        assigner=dict(
...
train_pipeline = [
    ...
    dict(type='FOVCrop', allow_negative_crop='True'),
    dict(type='CusRandomCrop', crop_size=(256, 256)),
    dict(
    ...
    dict(
        ...
        meta_keys=('ismask', 'filename', 'ori_filename', 'ori_shape',
...
data = dict(
    ...
    train=dict(
        ...
        dataset=dict(
            ...
            ann_file=f'{basedir}/train_annt_fold_{fold}.json',
...
# evaluation = dict(interval=10, metric=['bbox', 'segm', 'proposal'], by_epoch=by_epoch, save_best='pq', rule='greater')
evaluation = dict(interval=1, metric=['bbox', 'segm', 'proposal'], by_epoch=by_epoch, crop_inference_to_fov=False)
...
checkpoint_config = dict(interval=2, by_epoch=by_epoch, max_keep_ckpts=100)
log_config = dict(
    interval=50,
    hooks=[
