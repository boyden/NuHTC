...
class GeometricAugmentation(object):
    ...
    def _filter_invalid(self, results, min_size=0):
        ...
        for key in results.get("bbox_fields", []):
            ...
            #TODO Temperoaly featuren
            valid_inds = bbox_w*bbox_h > 10
        ...
        if 'ismask' in results:
            results['ismask'] = results['ismask'][valid_inds]
        return results
...
class RandTranslate(GeometricAugmentation):
    ...
    def _translate_seg(self, results, offset, direction="horizontal", fill_val=0):
        ...
        for key in results.get("seg_fields", []):
            ...
            results[key] = mmcv.imtranslate(seg, offset, direction, fill_val, interpolation='nearest').astype(
                seg.dtype
            )
    ...
    def _rotate_seg(self, results, angle, center=None, scale=1.0, fill_val=255):
        ...
        for key in results.get("seg_fields", []):
            ...
            results[key] = mmcv.imrotate(
                seg, angle, center, scale, border_value=fill_val, interpolation='nearest'
            ).astype(seg.dtype)
...
@PIPELINES.register_module()
class RecomputeBox(GeometricAugmentation):
    def __init__(self, min_size=10, record=False, **kwargs):
        super().__init__(**kwargs)
        self.record = record
        self.min_size = min_size
...
@PIPELINES.register_module()
class RandCorrupt(transforms.Corrupt):
    ...
    def __init__(self, corruption=None, severity=1, prob=0.5):
        if corruption == None:
            self.corruption = ['gaussian_noise', 'shot_noise', 'impulse_noise', 'defocus_blur',
                                'glass_blur', 'motion_blur', 'zoom_blur', 'snow', 'frost', 'fog',
                                'brightness', 'contrast', 'elastic_transform', 'pixelate', 'jpeg_compression',
                                'speckle_noise', 'gaussian_blur', 'spatter', 'saturate']
    ...
    def __call__(self, results):
        ...
        if np.random.random() < self.prob:
            results['img'] = corrupt(
                ...
                corruption_name=np.random.choice(self.corruption),
