...
def _resize_image_and_masks(image, scale_factor, target):
    ...
    image = F.interpolate(image[None], scale_factor=scale_factor, mode='bilinear', recompute_scale_factor=False, align_corners=False)[0]
...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def resize(self, image, target):
        ...
        image, target = _resize_image_and_masks(image=image, scale_factor=scale_factor, target=target)
    ...
    def __repr__(self):
        ...
        #                                                                                  self.max_size)
        format_string += "{0}Resize(scale_factor={1},mode='bilinear')".format(_indent, self.scale_factor)
        format_string += '\n)'
