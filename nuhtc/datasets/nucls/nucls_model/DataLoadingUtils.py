...
from .torchvision_detection_utils import transforms as tvdt  # noqa
from .torchvision_detection_utils.utils import collate_fn  # noqa
from ..configs.nucleus_style_defaults import Interrater, \
    DefaultAnnotationStyles, NucleusCategories  # noqa
from .DataFormattingUtils import from_dense_to_sparse_object_mask  # noqa
...
def save_cv_train_test_splits(
    ...
    slides = np.array(list({
        j.split('_id')[0] for j in sorted(os.listdir(opj(root, 'rgb')))
        if j.endswith('.png')}))
    if by_hospital:
...
class NucleusDatasetBase(object):
    def __init__(
        ...
        dbsqlite = connect_to_sqlite(self.dbpath)
        dbcon = dbsqlite.connect()
    ...
    def set_fovnames_and_ids(self, dbcon, slides: list):
        ...
        self.fovnames = [
            ...
            sorted(os.listdir(opj(self.root, 'rgb'))) if j.endswith('.png')
        ]
...
class NucleusDataset(NucleusDatasetBase):
    ...
    def getitem(self, idx, target=None, boxdf=None):
        ...
        rgb_path = opj(self.root, "rgb", f"{fovname}.png")
