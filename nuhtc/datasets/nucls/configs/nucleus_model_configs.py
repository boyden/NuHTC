...
from .nucleus_style_defaults import NucleusCategories  # noqa
...
class CoreSetQC(object):
    ...
    dname = 'FINAL_CORE'
    dataset_root = opj(BASEPATH, 'QC')
    dbpath = opj(BASEPATH, f'{BASEPATH}/{dname}.sqlite')
...
class CoreSetNoQC(object):
    ...
    dataset_root = opj(BASEPATH, 'noQC')
    dbpath = opj(BASEPATH, f'{BASEPATH}/{dname}.sqlite')
...
class RawEvalSets(object):
    ...
    for ev in ['E', 'B-control', 'U-control']:
        dataset_roots[ev] = opj(BASEPATH, f'{BASEPATH}', ev)
...
class FasterRCNNConfigs(object):
    ...
    training_params = {
        ...
        # monitoring params
        'print_freq': 100,  # meaningful if >= effective_batch_size / batch_size
        'window_size': None,
        'smoothing_window': 25,
