...
####
def get_coord_pq(true, pred, match_iou=0.5):
    """
    `match_iou` is the IoU threshold level to determine the pairing between
    GT instances `p` and prediction instances `g`. `p` and `g` is a pair
    if IoU > `match_iou`. However, pair of `p` and `g` must be unique
    ...
    if len(pred_id_list) == 1:
        return [0, [], true_id_list[1:], []]
    true_coord = np.array([center_of_mass(true==i) for i in true_id_list[1:]])
    pred_coord = np.array([center_of_mass(pred==i) for i in pred_id_list[1:]])

    paired_true, unpaired_true, unpaired_pred = pair_coordinates(true_coord, pred_coord, 12)
    ...
    return [dq, paired_true, unpaired_true, unpaired_pred]
#####