...
from wsi_core.WholeSlideImage import WholeSlideImage, Dataset_All_Bags, Whole_Slide_Bag_FP, Dataset_All_Bags_DF
...
def seg_and_patch(source, save_dir, patch_save_dir, mask_save_dir, stitch_save_dir,
                  ...
                  stitch=False,
                  patch=False, auto_skip=True, process_list=None, target_spacing=0.25):
    ...
    for i in range(total):
        ...
        mag = WSI_object.getMag()
        if mag != 'NA':
            mag_approximation = [20, 40, 60, 80][np.argmin([abs(x - mag) for x in [20, 40, 60, 80]])]
        else:
            mag_approximation == 'NA'

        df.loc[idx, 'magnification'] = mag
        df.loc[idx, 'mag_approximation'] = mag_approximation
        ...
        if patch:
            current_patch_params.update({'patch_level': patch_level, 'patch_size': patch_size, 'step_size': step_size,
                                         'save_path': patch_save_dir, 'target_spacing': target_spacing})
...
# python tools/infer_wsi.py demo/wsi configs/nuhtc/htc_lite_swin_pytorch_fpn_PanNuke_seasaw_CAS.py models/ema_pannuke.pth --out demo/wsi_res --patch --seg --stitch --space 256 --step_size 192 --target_spacing 0.25 --margin 2 --min_area 10 --save_dir demo/wsi_infer --mode merge --no_auto_skip
...
def parse_args():
    ...
    parser.add_argument('--space', type=float, default=0.25,
                        help='target_spacing to patch, only work if patch_level < 0')
...
def main():
    ...
    seg_times, patch_times = seg_and_patch(**directories, **parameters,
                                           ...
                                           target_spacing=args.space)

    csv_path = f'{args.save_dir}/process_list_autogen.csv'
    bag_df = pd.read_csv(csv_path, index_col=None)
    mag_li = [40, 20]
    for i, mag in enumerate(mag_li):
        batch_size = int(batch_size_default / 2 ** i)
        min_area = args.min_area / 2 ** i
        bag_mag_df = bag_df[bag_df['mag_approximation'] == mag]
        if len(bag_mag_df) == 0:
            continue
        bags_dataset = Dataset_All_Bags_DF(bag_mag_df)
        total = len(bags_dataset)
        cfg_basename = os.path.splitext(args.config)[0]
        cfg = Config.fromfile(f'{cfg_basename}_{mag}.py')
        ...
        model.inst_fillColor = ["rgba(255, 0, 0, 0)", "rgba(0, 255, 0, 0)", "rgba(0, 0, 255, 0)",
                                "rgba(255, 255, 0, 0)", "rgba(255, 0, 255, 0)"]
        model.inst_lineColor = ["rgb(255, 0, 0)", "rgb(0, 255, 0)", "rgb(0, 0, 255)", "rgb(255, 255, 0)",
                                "rgb(255, 0, 255)"]
        ...
        for bag_candidate_idx in range(total):
            ...
            if os.path.exists(f'{args.save_dir}/patches_crop'):
                h5_file_path = os.path.join(args.save_dir, 'patches_crop', bag_name)
            else:
            ...
            db_name = f'{args.save_dir}/{slide_id}/contour_{args.patch_size}.db'
            if os.path.exists(db_name):
                if os.path.getsize(db_name) / (1024 ** 2) < 1:
                    os.remove(db_name)
                else:
                    continue
            else:
                ...
                c = conn.cursor()
                c.execute('''CREATE TABLE contour
                                       (id         INTEGER PRIMARY KEY AUTOINCREMENT,
                                       annidx      INT NOT NULL,
                                       elementidx  INT NOT NULL,
                                       type        VARCHAR(50),
                                       "group"     VARCHAR(50),
                                       score       FLOAT,
                                       color       TEXT,
            ...
            contours_df = pd.DataFrame(
                columns=["annidx", "elementidx", "type", "group", "color", "xmin", "xmax", "ymin", "ymax", "bbox_area",
                         "coords_x", "coords_y"])
            ...
            for idx, data in enumerate(tqdm(infer_dataloader)):
                ...
                for mask_id in range(len(seg_mask)):
                    ...
                    select_id = (bbox_results[mask_id][:, 0] >= margin_filter) & (
                            bbox_results[mask_id][:, 1] >= margin_filter) & (
                                        bbox_results[mask_id][:, 2] <= img_shape[1] - margin_filter) & (
                                        bbox_results[mask_id][:, 3] <= img_shape[0] - margin_filter)
                    select_id = select_id & (seg_area > min_area)
                    ...
                    bbox_areas = (bbox_results[mask_id][:, 2] - bbox_results[mask_id][:, 0]) * (
                            bbox_results[mask_id][:, 3] - bbox_results[mask_id][:, 1])
                    labels[mask_id] = labels[mask_id][select_id]
                    fg_scores[mask_id] = fg_scores[mask_id][select_id]
                    ...
                    if args.mode == 'dsa' or args.mode == 'all':
                        dsajson = [{
                            ...
                            "type": "polyline",
                            "closed": True,
                            "points": np.pad(seg_mask[mask_id][i][0], ((0, 0), (0, 1),), 'constant',
                        ...
                        dsajson_li += dsajson
                    if args.mode == 'merge' or args.mode == 'all':
                        for i in range(len(seg_mask[mask_id])):
                            con_dict = {
                                "annidx": annidx,
                                "elementidx": i,
                                ...
                                "bbox_area": (seg_mask[mask_id][i][0, :, 0].max() - seg_mask[mask_id][i][0, :,
                                                                                    0].min()) * (
                                                     seg_mask[mask_id][i][0, :, 1].max() - seg_mask[mask_id][i][0,
                                                                                           :, 1].min()),
                        ...
                        # contours_df = pd.concat([contours_df, con_df], ignore_index=True)
                if idx % 1000 == 0 or idx == len(infer_dataloader) - 1:
                    ...
                    if args.mode == 'merge' or args.mode == 'sql' or args.mode == 'all':
                        conn.commit()
                        conn.close()
                        conn = sqlite3.connect(f'{args.save_dir}/{slide_id}/contour_{args.patch_size}.db')
                        c = conn.cursor()
                        # contours_df.to_csv(f'{args.save_dir}/{slide_id}/{slide_id}_contour_{args.patch_size}.csv', index=False)
                        if idx == len(infer_dataloader) - 1:
                            # if os.path.exists(f'{args.save_dir}/{slide_id}/contour_{args.patch_size}.db'):
                            #     os.remove(f'{args.save_dir}/{slide_id}/contour_{args.patch_size}.db')
                            # conn = sqlite3.connect(f'{args.save_dir}/{slide_id}/contour_{args.patch_size}.db')
                            # contours_df.to_sql(name='contour', con=conn, if_exists='replace', index_label='id')
                            # c = conn.cursor()
